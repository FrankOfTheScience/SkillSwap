...
public class CreateBookingCommandHandlerTests : IDisposable
{
    ...
    private readonly Guid _testOfferId;
    ...
    public CreateBookingCommandHandlerTests()
    {
        ...
        _testOfferId = Guid.NewGuid();

        // Seed test data
        var testUser = new User { Id = _testUserId, Email = "test@example.com", DisplayName = "Test User", PasswordHash = "hash" };
        var testOffer = new Offer { Id = _testOfferId, Price = 100.00m, Title = "Test Offer", Description = "Test Description", CreatedBy = _testUserId };
    ...
    public async Task Handle_ValidCommand_ShouldCreateBooking()
    {
        ...
        var command = new CreateBookingCommand(OfferId: _testOfferId, UserId: user.Id);
        ...
        createdBooking!.OfferId.Should().Be(_testOfferId);
    ...
    public async Task Handle_InvalidValidation_ShouldThrowValidationException()
    {
        ...
        var command = new CreateBookingCommand(OfferId: _testOfferId, UserId: Guid.NewGuid());
    ...
    public async Task Handle_NonExistentOffer_ShouldThrowInvalidOperationException()
    {
        ...
        var command = new CreateBookingCommand(OfferId: nonExistentOfferId, UserId: user.Id); // Non-existent offer

        // Act & Assert
        var act = async () => await handler.Handle(command, CancellationToken.None);
        await act.Should().ThrowAsync<InvalidOperationException>()
            .WithMessage($"*Offer with ID {nonExistentOfferId} not found*");
    }
    ...
    public async Task Handle_NonExistentUser_ShouldThrowInvalidOperationException()
    {
        ...
        var command = new CreateBookingCommand(OfferId: _testOfferId, UserId: nonExistentUserId);
